...
| Goal | Command |
|---|---|
| Standard dev install (linter, mypy, pytest, pytest-qt, `anyio[trio]`) | `uv sync --extra dev` |
| Add FLIR IR camera support (requires sibling `capa-flir/`) | `uv sync --extra dev --extra flir` |
| Add docs tooling (zensical + mkdocstrings) | `uv sync --group docs` |
| Everything at once | `uv sync --all-extras --group docs` |

The `dev` extra is declared at [pyproject.toml:74-82](../../pyproject.toml#L74-L82); the `docs` group at [pyproject.toml:85-89](../../pyproject.toml#L85-L89).
...
The repo deliberately ships no `.pre-commit-config.yaml`. The four-command dev loop above is the contract; wire it as an editor save action or shell alias if you want it automated locally. Don't add a pre-commit config without discussing it first — it interacts badly with the project's preference for occasional bulk-style commits (see [Commits and PRs](commit-and-pr.md)).
