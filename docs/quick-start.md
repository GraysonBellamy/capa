...
has the event log, and `samples.parquet` has the channel data.
...
- The [status bar guide](status-bar-guide.md) explains every pill in
  the diagnostic strip — `sat`, `loop`, `q`, etc.
