...
The status bar polls live metrics from the conductor and UI ring buffers at 1 Hz. Every pill is designed to mean something **right now** — counters that grow by-design or percentiles over stale windows have been deliberately removed. See `src/capa/ui/statusbar.py` for the implementation and [`runtime-architecture.md`](runtime-architecture.md) for the runtime concepts each pill exposes.
...
**What it shows.** Total ring rollovers summed across every channel's UI ring buffer ([`ringbuffer.py:total_overflow`](../src/capa/core/ringbuffer.py)). Each rollover = a new sample arrived while the ring was already at capacity, so the oldest sample was evicted to make room.
...
**Sizing.** [`RingBufferRegistry.register`](../src/capa/core/ringbuffer.py) defaults to enough capacity to hold `DEFAULT_HISTORY_S` (10 min) of samples at the channel's `decimate_to_hz`. So under normal config a 50 Hz balance ring holds ~30 000 samples and only starts rolling over after ten minutes of run time.
...
**Fail (red).** N ≥ 50% of `saturation_deadline_s` (≥ 5 s by default). The run is in real danger of being sealed as `crashed_but_sealed` ([runtime-architecture.md §6.3](runtime-architecture.md#63-saturation-deadline)).

**What to check, in order.**
1. **Loop lag pill** — high conductor loop lag means the loop itself is CPU-starved; nothing downstream gets done. Likely a `CustomStep` procedure handler doing inline CPU work, see [runtime-architecture.md §11](runtime-architecture.md#11-procedure-cpu-offload).
